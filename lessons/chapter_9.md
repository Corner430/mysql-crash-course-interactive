...
9️⃣  定位符
   ...
   -- 匹配: .5 ton anvil, 1 ton anvil

   示例：完全匹配
   WHERE prod_name REGEXP '^Safe$'
   -- 只匹配与'Safe'完全相同的产品


   💡 词边界（高级功能）
   ────────────────────────────────────
   词边界用于匹配独立的单词，而非单词的一部分。

   什么是"独立的词"？
   - "ton" 在 ".5 ton anvil" 中是独立的词。
   - "ton" 在 "Detonator" 中不是独立的词（是单词的一部分）。

   不同MySQL版本的语法：

   MySQL 8.0+ 语法（推荐）：
   \\b - 词边界
   示例：WHERE prod_name REGEXP '\\bton\\b'
   ⚠️ 注意：在SQL字符串中，`\`是转义符，所以`\b`要写成`\\b`。

   MySQL 5.x 旧语法（MySQL 8.0已废弃）：
   [[:<:]] - 词的开始
   [[:>:]] - 词的结束
   示例：WHERE prod_name REGEXP '[[:<:]]ton[[:>:]]'

   由于版本兼容性问题，此功能在实际开发中没有 ^ 和 $ 常用。
...
示例 3: 字符集匹配
  ...
  结果：
  .5 ton anvil  -- 不匹配（前面的字符'5'不在集合[123]中）
  1 ton anvil   -- 匹配
  2 ton anvil   -- 匹配
