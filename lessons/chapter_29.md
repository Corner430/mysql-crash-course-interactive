━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📚 第 29 章：数据库维护
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📖 学习目标
──────────────────────────────────────
✓ 了解数据库备份的重要性
✓ 掌握常用的维护命令
✓ 学会诊断和修复表
✓ 理解日志文件的作用

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📝 核心知识点
──────────────────────────────────────

1️⃣  为什么需要维护
   ────────────────────────────────────
   数据库维护确保：
   ✓ 数据安全（备份和恢复）
   ✓ 性能优化（索引优化）
   ✓ 问题诊断（检查和修复）
   ✓ 日志管理（监控和审计）


2️⃣  备份数据
   ────────────────────────────────────
   备份的重要性：
   • 硬件故障
   • 软件bug
   • 人为错误
   • 安全攻击
   
   备份策略：
   • 定期备份（每日/每周）
   • 多地存储
   • 测试恢复


3️⃣  mysqldump - 逻辑备份
   ────────────────────────────────────
   备份单个数据库：
   mysqldump -u root -p crashcourse > backup.sql
   
   备份所有数据库：
   mysqldump -u root -p --all-databases > all_backup.sql
   
   备份特定表：
   mysqldump -u root -p crashcourse customers orders
   > tables_backup.sql
   
   恢复备份：
   mysql -u root -p crashcourse < backup.sql


4️⃣  ANALYZE TABLE - 分析表
   ────────────────────────────────────
   ANALYZE TABLE table_name;
   
   作用：
   • 分析表的关键字分布
   • 更新索引统计信息
   • 帮助优化器做出更好的决策
   
   示例：
   ANALYZE TABLE customers;
   ANALYZE TABLE products, orders;


5️⃣  CHECK TABLE - 检查表
   ────────────────────────────────────
   CHECK TABLE table_name;
   
   作用：
   • 检查表是否存在错误
   • 检查索引是否正确
   • 检查数据文件
   
   示例：
   CHECK TABLE customers;
   CHECK TABLE products, orders;


6️⃣  OPTIMIZE TABLE - 优化表
   ────────────────────────────────────
   OPTIMIZE TABLE table_name;
   
   作用：
   • 回收未使用的空间
   • 整理数据碎片
   • 更新索引统计
   
   示例：
   OPTIMIZE TABLE customers;
   
   注意：
   • 对 InnoDB 表，相当于 ALTER TABLE ... ENGINE=InnoDB
   • 会锁定表
   • 大表可能耗时很长


7️⃣  REPAIR TABLE - 修复表
   ────────────────────────────────────
   REPAIR TABLE table_name;
   
   作用：
   • 修复损坏的表
   • 主要用于 MyISAM 表
   • InnoDB 通常自动修复
   
   示例：
   REPAIR TABLE customers;


8️⃣  查看服务器状态
   ────────────────────────────────────
   查看运行状态：
   SHOW STATUS;
   
   查看特定状态：
   SHOW STATUS LIKE 'Threads%';
   SHOW STATUS LIKE 'Uptime';
   SHOW STATUS LIKE 'Slow_queries';
   
   查看进程列表：
   SHOW PROCESSLIST;


9️⃣  日志文件
   ────────────────────────────────────
   错误日志：
   • 记录启动、运行、停止的问题
   • 通常在数据目录
   
   查询日志：
   • 记录所有SQL语句
   • 用于调试
   • 性能影响大，一般不开启
   
   慢查询日志：
   • 记录执行缓慢的查询
   • 用于性能优化
   
   二进制日志：
   • 记录数据变更
   • 用于复制和恢复


🔟  查看日志设置
   ────────────────────────────────────
   查看错误日志位置：
   SHOW VARIABLES LIKE 'log_error';
   
   查看查询日志设置：
   SHOW VARIABLES LIKE 'general_log%';
   
   查看慢查询日志：
   SHOW VARIABLES LIKE 'slow_query%';
   
   查看二进制日志：
   SHOW VARIABLES LIKE 'log_bin%';

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎯 实践任务
──────────────────────────────────────

📝 基础练习
──────────────────────────────────────

练习 1: 分析表
──────────────────────────────────────
分析 customers 表

你的SQL：
  ANALYZE TABLE customers;


练习 2: 检查表
──────────────────────────────────────
检查 orders 表是否有错误

你的SQL：
  CHECK TABLE orders;


练习 3: 查看服务器状态
──────────────────────────────────────
查看MySQL运行时间

你的SQL：
  SHOW STATUS LIKE 'Uptime';


练习 4: 查看慢查询数量
──────────────────────────────────────
查看有多少慢查询

你的SQL：
  SHOW STATUS LIKE 'Slow_queries';


练习 5: 查看当前进程
──────────────────────────────────────
查看正在执行的查询

你的SQL：
  SHOW PROCESSLIST;


练习 6: 查看日志设置
──────────────────────────────────────
查看错误日志位置

你的SQL：
  SHOW VARIABLES LIKE 'log_error';

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🚀 进阶挑战
──────────────────────────────────────

挑战 1: 完整的表维护流程
──────────────────────────────────────
对表执行完整维护

你的SQL：
  -- 1. 检查表
  CHECK TABLE customers;
  
  -- 2. 分析表
  ANALYZE TABLE customers;
  
  -- 3. 优化表（注意：会锁表）
  -- OPTIMIZE TABLE customers;


挑战 2: 批量维护
──────────────────────────────────────
维护多个表

你的SQL：
  -- 分析所有核心表
  ANALYZE TABLE customers, orders, products, vendors;
  
  -- 检查所有核心表
  CHECK TABLE customers, orders, products, vendors;


挑战 3: 查看表大小
──────────────────────────────────────
查看每个表占用的空间

你的SQL：
  SELECT 
      table_name AS 'Table',
      ROUND(((data_length + index_length) / 1024 / 1024), 2) 
      AS 'Size (MB)'
  FROM information_schema.TABLES
  WHERE table_schema = 'crashcourse'
  ORDER BY (data_length + index_length) DESC;


挑战 4: 监控连接数
──────────────────────────────────────
查看连接相关信息

你的SQL：
  SHOW STATUS LIKE 'Threads%';
  SHOW STATUS LIKE 'Connections';
  SHOW STATUS LIKE 'Max_used_connections';

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

💡 重要提示
──────────────────────────────────────

⚠️  备份至关重要
   • 定期备份是第一要务
   • 测试备份的恢复
   • 多地存储备份文件
   • 自动化备份流程

⚠️  维护窗口
   • 选择低峰时段维护
   • OPTIMIZE 会锁定表
   • 提前通知用户
   • 准备回滚方案

⚠️  InnoDB vs MyISAM
   InnoDB：
   • 自动崩溃恢复
   • OPTIMIZE 等于重建表
   • 更可靠
   
   MyISAM：
   • 需要手动 REPAIR
   • OPTIMIZE 更有效
   • 较老的引擎

⚠️  日志管理
   • 查询日志影响性能
   • 只在调试时开启
   • 慢查询日志很有用
   • 定期清理旧日志

⚠️  监控指标
   重要指标：
   • Uptime：运行时间
   • Connections：连接数
   • Slow_queries：慢查询数
   • Threads_connected：当前连接
   • Questions：查询总数

⚠️  性能影响
   维护命令可能：
   • 锁定表
   • 消耗CPU和I/O
   • 影响正常业务
   • 需要在低峰期执行

⚠️  自动化
   使用脚本自动化：
   • 定期备份
   • 定期分析表
   • 监控告警
   • 日志轮转

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📊 实战示例
──────────────────────────────────────

示例 1: 备份数据库（命令行）
  # 备份单个数据库
  mysqldump -u root -p crashcourse > crash_backup.sql
  
  # 备份所有数据库
  mysqldump -u root -p --all-databases > all_backup.sql
  
  # 压缩备份
  mysqldump -u root -p crashcourse | gzip > crash_backup.sql.gz


示例 2: 恢复数据库（命令行）
  # 恢复数据库
  mysql -u root -p crashcourse < crash_backup.sql
  
  # 从压缩文件恢复
  gunzip < crash_backup.sql.gz | mysql -u root -p crashcourse


示例 3: 分析表
  ANALYZE TABLE customers;
  ANALYZE TABLE products, orders, orderitems;


示例 4: 检查表
  CHECK TABLE customers;
  CHECK TABLE customers EXTENDED;  -- 更彻底的检查


示例 5: 优化表
  OPTIMIZE TABLE customers;
  -- 注意：会锁定表，大表耗时可能很长


示例 6: 查看服务器状态
  SHOW STATUS;
  SHOW STATUS LIKE 'Uptime';
  SHOW STATUS LIKE 'Slow_queries';
  SHOW STATUS LIKE 'Threads%';


示例 7: 查看进程
  SHOW PROCESSLIST;
  SHOW FULL PROCESSLIST;  -- 显示完整SQL


示例 8: 查看表大小
  SELECT 
      table_name,
      ROUND(((data_length + index_length) / 1024 / 1024), 2) 
      AS 'Size (MB)'
  FROM information_schema.TABLES
  WHERE table_schema = 'crashcourse';


示例 9: 查看数据库大小
  SELECT 
      table_schema AS 'Database',
      SUM(data_length + index_length) / 1024 / 1024 
      AS 'Size (MB)'
  FROM information_schema.TABLES
  GROUP BY table_schema;


示例 10: 查看索引使用情况
  SHOW INDEX FROM customers;

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎓 知识检查点
──────────────────────────────────────

□ 你理解备份的重要性吗？
□ 你知道如何使用 mysqldump 吗？
□ 你会使用 ANALYZE TABLE 吗？
□ 你会使用 CHECK TABLE 吗？
□ 你了解 OPTIMIZE TABLE 的作用吗？
□ 你知道如何查看服务器状态吗？
□ 你了解不同类型的日志吗？
□ 你知道维护命令的性能影响吗？

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🔍 维护最佳实践
──────────────────────────────────────

1. 备份策略
   ✓ 每日增量备份
   ✓ 每周完整备份
   ✓ 异地存储备份
   ✓ 定期测试恢复
   ✓ 自动化备份脚本

2. 定期维护
   ✓ 每周分析关键表
   ✓ 每月检查所有表
   ✓ 季度优化大表
   ✓ 在低峰期执行

3. 监控
   ✓ 监控慢查询
   ✓ 监控连接数
   ✓ 监控磁盘空间
   ✓ 设置告警

4. 日志管理
   ✓ 启用慢查询日志
   ✓ 定期分析日志
   ✓ 定期清理旧日志
   ✓ 归档重要日志

5. 性能优化
   ✓ 定期分析慢查询
   ✓ 优化表结构
   ✓ 添加合适的索引
   ✓ 清理无用数据

6. 文档记录
   ✓ 记录维护计划
   ✓ 记录变更历史
   ✓ 记录问题和解决方案
   ✓ 更新操作手册

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📋 维护检查清单
──────────────────────────────────────

每日：
□ 检查备份是否成功
□ 检查错误日志
□ 监控磁盘空间

每周：
□ 分析关键表
□ 检查慢查询日志
□ 审查连接数

每月：
□ 检查所有表
□ 优化必要的表
□ 审查用户权限
□ 清理旧日志

每季度：
□ 测试备份恢复
□ 审查安全设置
□ 性能基准测试
□ 容量规划

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✨ 本章小结
──────────────────────────────────────

你已经学会了：
✓ 理解数据库维护的重要性
✓ 使用 mysqldump 备份和恢复
✓ 使用 ANALYZE TABLE 分析表
✓ 使用 CHECK TABLE 检查表
✓ 使用 OPTIMIZE TABLE 优化表
✓ 查看服务器状态和进程
✓ 了解不同类型的日志
✓ 掌握维护最佳实践

核心要点：
• 备份是最重要的维护任务
• 定期分析和检查表保持健康
• OPTIMIZE TABLE 会锁表，谨慎使用
• 监控服务器状态和慢查询
• 制定维护计划并严格执行

下一章（最后一章）将学习改善性能！

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

💻 动手实践
──────────────────────────────────────
安全的维护命令：

-- 分析表
ANALYZE TABLE customers;

-- 检查表
CHECK TABLE customers;

-- 查看服务器状态
SHOW STATUS;
SHOW STATUS LIKE 'Uptime';
SHOW STATUS LIKE 'Slow_queries';

-- 查看进程
SHOW PROCESSLIST;

-- 查看表大小
SELECT 
    table_name,
    ROUND(((data_length + index_length) / 1024 / 1024), 2) 
    AS 'Size (MB)'
FROM information_schema.TABLES
WHERE table_schema = 'crashcourse';

⚠️  不要在生产环境随意执行 OPTIMIZE
    它会锁定表，影响正常业务

数据库维护是保证系统稳定运行的关键！

选择 "1. 进入 MySQL 实践" 开始练习
